...
class PlanningGraph():
        ...
        for action in self.all_actions:
            ...
            if precond_s_nodes.issubset(self.s_levels[level]):                
                ...
                for s_node in self.s_levels[level]:
                    # connect all the children and parents relationship
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        n1_pos_effs = set(node_a1.action.effect_add)
        n1_neg_effs = set(node_a1.action.effect_rem)
        n2_pos_effs = set(node_a2.action.effect_add)
        n2_neg_effs = set(node_a2.action.effect_rem)

        return (len(n1_pos_effs.intersection(n2_neg_effs)) > 0) or (len(n2_pos_effs.intersection(n1_neg_effs)) > 0)
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        n1_rem_effs = set(node_a1.action.effect_rem)
        n2_rem_effs = set(node_a2.action.effect_rem)
        n1_preconds = set(node_a1.action.precond_pos)
        n2_preconds = set(node_a2.action.precond_pos)

        return (len(n1_rem_effs.intersection(n2_preconds)) > 0) or (len(n2_rem_effs.intersection(n1_preconds)) > 0)
